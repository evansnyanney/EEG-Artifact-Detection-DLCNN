...
__all__ = ['MuscleArtifactDetector', 'F1EarlyStopping']


class F1EarlyStopping(callbacks.Callback):
    """
    Monitor F1 score and stop training when it fails to improve for ``patience`` epochs.

    Expects ``monitor`` (e.g. ``val_f1``) to be present in ``logs`` each epoch.
    """
...
class MuscleArtifactDetector:
    ...
    def __init__(self, model_name: str = "edl_cnn_muscle", verbose: bool = True) -> None:
    ...
    def load_data(
        self, data_dir: str = "binary_models_data/muscle_artifacts"
    ) -> Tuple[
    ...
    def build_model(
        ...
        """
        Build a CNN for muscle artifact detection.
        ...
        if self.metadata is None:
            raise ValueError("Metadata not loaded. Call load_data() before build_model().")
    ...
    def train(self, epochs: int = 200, batch_size: int = 128) -> tf.keras.callbacks.History:
        """
        Train the model with learning-rate reduction, checkpointing, and early stopping.
    ...
    def evaluate(self) -> Dict[str, Any]:
        ...
        print(classification_report(
            y_true_test, y_pred_test, target_names=['Clean', 'Muscle Artifact']
        ))
        print("Confusion Matrix:")
        print(cm)

        self._save_evaluation_plots(
            fpr, tpr, roc_auc, precision_val, recall_val,
    ...
    def _plot_pr_curve(
        self, recall: np.ndarray, precision: np.ndarray, thr: float, idx: int
    ) -> None:
        ...
        plt.title('PR Curve (Validation) - F1-Optimal Threshold - Muscle Artifact Detector')
        plt.legend()
        plt.grid(True)
        plt.savefig(
            os.path.join(self.results_dir, f"{self.model_name}_pr_curve.png"),
    ...
    def _save_evaluation_plots(
        ...
        axes[0, 0].set_title('ROC Curve (Validation) - Muscle Artifact Detector')
        axes[0, 0].legend()
        ...
        axes[0, 1].set_title('PR Curve (Validation) - Muscle Artifact Detector')
        axes[0, 1].legend()
        ...
        axes[1, 0].set_title(f'Confusion Matrix - Muscle Artifact Detector\nThreshold = {threshold:.3f}')
        ...
        axes[1, 1].hist(
            ...
            label='Muscle Artifact', color='orange'
        )
        axes[1, 1].axvline(threshold, color='red', linestyle='--', label=f'Threshold = {threshold:.3f}')
        axes[1, 1].set_xlabel('Prediction Probability')
        axes[1, 1].set_ylabel('Density')
        axes[1, 1].set_title('Prediction Distribution (Test) - Muscle Artifact Detector')
        axes[1, 1].legend()
    ...
    def plot_confusion_matrix(self, y_true: np.ndarray, y_pred: np.ndarray, threshold: float) -> None:
        ...
        plt.title(
            f'Confusion Matrix - Muscle Artifact Detector\nThreshold = {threshold:.3f}',
    ...
    def plot_training_history(self) -> None:
        ...
        for i, (metric, title, tc, vc) in enumerate(metrics):
            ...
            ax.set_title(f'Model {title} - Muscle Artifact Detector', fontweight='bold')
            ax.set_xlabel('Epoch')
            ax.set_ylabel(title)
            ax.legend()
...
def main() -> None:
    """Train and evaluate the muscle artifact detector (CLI entry point)."""
